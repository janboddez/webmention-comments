...
class Tokenizer
{
    ...
    {
        $this->textMode = $textmode & (\Webmention_Comments\Masterminds\HTML5\Elements::TEXT_RAW | \Webmention_Comments\Masterminds\HTML5\Elements::TEXT_RCDATA);
    ...
    protected function consumeData()
    {
        ...
        if (\false === $tok) {
            ...
            switch ($this->textMode) {
                case \Webmention_Comments\Masterminds\HTML5\Elements::TEXT_RAW:
                    $this->rawText($tok);
                    break;
                case \Webmention_Comments\Masterminds\HTML5\Elements::TEXT_RCDATA:
                    $this->rcdata($tok);
                    break;
                default:
                    if ('<' === $tok || '&' === $tok) {
    ...
    protected function rcdata($tok)
    {
        ...
        $sequence = '</' . $this->untilTag;
        $txt = '';
        $caseSensitive = !\Webmention_Comments\Masterminds\HTML5\Elements::isHtml5Element($this->untilTag);
    ...
    protected function tagName()
    {
        ...
        } catch (\Webmention_Comments\Masterminds\HTML5\Parser\ParseError $e) {
            $selfClose = \false;
        }
    ...
    protected function attribute(&$attributes)
    {
        ...
        if ('<' == $tok) {
            ...
            throw new \Webmention_Comments\Masterminds\HTML5\Parser\ParseError('Start tag inside of attribute.');
        }
    ...
    protected function comment()
    {
        ...
        if ("\0" == $tok) {
            $tok = \Webmention_Comments\Masterminds\HTML5\Parser\UTF8Utils::FFFD;
        }
    ...
    protected function doctype()
    {
        ...
        // EOF: die.
        if (\false === $tok) {
            $this->events->doctype('html5', \Webmention_Comments\Masterminds\HTML5\Parser\EventHandler::DOCTYPE_NONE, '', \true);
            $this->eof();
            return \true;
        }
        // NULL char: convert.
        ...
        $doctypeName = \strtolower(\strtr($doctypeName, "\0", \Webmention_Comments\Masterminds\HTML5\Parser\UTF8Utils::FFFD));
        $tok = $this->scanner->current();
        // If false, emit a parse error, DOCTYPE, and return.
        if (\false === $tok) {
            $this->parseError('Unexpected EOF in DOCTYPE declaration.');
            $this->events->doctype($doctypeName, \Webmention_Comments\Masterminds\HTML5\Parser\EventHandler::DOCTYPE_NONE, null, \true);
            return \true;
        }
        ...
        if (('PUBLIC' == $pub || 'SYSTEM' == $pub) && $white > 0) {
            // Get the sys ID.
            $type = 'PUBLIC' == $pub ? \Webmention_Comments\Masterminds\HTML5\Parser\EventHandler::DOCTYPE_PUBLIC : \Webmention_Comments\Masterminds\HTML5\Parser\EventHandler::DOCTYPE_SYSTEM;
    ...
    protected function decodeCharacterReference($inAttribute = \false)
    {
        ...
        if ('#' === $tok) {
            ...
            if ('x' === $tok || 'X' === $tok) {
                ...
                $entity = \Webmention_Comments\Masterminds\HTML5\Parser\CharacterReference::lookupHex($hex);
            } else {
                ...
                $entity = \Webmention_Comments\Masterminds\HTML5\Parser\CharacterReference::lookupDecimal($numeric);
            }
            ...
            $entity = \Webmention_Comments\Masterminds\HTML5\Parser\CharacterReference::lookupName($cname);
