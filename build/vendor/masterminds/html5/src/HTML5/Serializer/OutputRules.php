 ...
 */
namespace Webmention_Comments\Masterminds\HTML5\Serializer;

use Webmention_Comments\Masterminds\HTML5\Elements;
/**
 * Generate the output html5 based on element rules.
 */
class OutputRules implements \Webmention_Comments\Masterminds\HTML5\Serializer\RulesInterface
{
    /**
    ...
    public function setTraverser(\Webmention_Comments\Masterminds\HTML5\Serializer\Traverser $traverser)
    {
    ...
    public function element($ele)
    {
        ...
        if ('svg' == $name) {
            ...
            $name = \Webmention_Comments\Masterminds\HTML5\Elements::normalizeSvgElement($name);
        } elseif ('math' == $name) {
            $this->outputMode = static::IM_IN_MATHML;
        }
        $this->openTag($ele);
        if (\Webmention_Comments\Masterminds\HTML5\Elements::isA($name, \Webmention_Comments\Masterminds\HTML5\Elements::TEXT_RAW)) {
    ...
    {
        if (isset($ele->parentNode) && isset($ele->parentNode->tagName) && \Webmention_Comments\Masterminds\HTML5\Elements::isA($ele->parentNode->localName, \Webmention_Comments\Masterminds\HTML5\Elements::TEXT_RAW)) {
            $this->wr($ele->data);
            return;
        }
    ...
    protected function attrs($ele)
    {
        ...
        for ($i = 0; $i < $len; ++$i) {
            ...
            if ($this->outputMode == static::IM_IN_SVG) {
                $name = \Webmention_Comments\Masterminds\HTML5\Elements::normalizeSvgAttribute($name);
            } elseif ($this->outputMode == static::IM_IN_MATHML) {
                $name = \Webmention_Comments\Masterminds\HTML5\Elements::normalizeMathMlAttribute($name);
            }
            $this->wr(' ')->wr($name);
    ...
    protected function enc($text, $attribute = \false)
    {
        ...
        if ($this->hasHTML5) {
            ...
            return \strtr($text, \Webmention_Comments\Masterminds\HTML5\Serializer\HTML5Entities::$map);
        }
    }
    /**
     * Escape test.
