...
namespace Webmention_Comments\Masterminds;

use Webmention_Comments\Masterminds\HTML5\Parser\DOMTreeBuilder;
use Webmention_Comments\Masterminds\HTML5\Parser\Scanner;
use Webmention_Comments\Masterminds\HTML5\Parser\Tokenizer;
use Webmention_Comments\Masterminds\HTML5\Serializer\OutputRules;
use Webmention_Comments\Masterminds\HTML5\Serializer\Traverser;
/**
...
class HTML5
{
    ...
    public function parse($input, array $options = array())
    {
        ...
        $events = new \Webmention_Comments\Masterminds\HTML5\Parser\DOMTreeBuilder(\false, $options);
        $scanner = new \Webmention_Comments\Masterminds\HTML5\Parser\Scanner($input, !empty($options['encoding']) ? $options['encoding'] : 'UTF-8');
        $parser = new \Webmention_Comments\Masterminds\HTML5\Parser\Tokenizer($scanner, $events, !empty($options['xmlNamespaces']) ? \Webmention_Comments\Masterminds\HTML5\Parser\Tokenizer::CONFORMANT_XML : \Webmention_Comments\Masterminds\HTML5\Parser\Tokenizer::CONFORMANT_HTML);
        $parser->parse();
    ...
    {
        $options = \array_merge($this->defaultOptions, $options);
        $events = new \Webmention_Comments\Masterminds\HTML5\Parser\DOMTreeBuilder(\true, $options);
        $scanner = new \Webmention_Comments\Masterminds\HTML5\Parser\Scanner($input, !empty($options['encoding']) ? $options['encoding'] : 'UTF-8');
        $parser = new \Webmention_Comments\Masterminds\HTML5\Parser\Tokenizer($scanner, $events, !empty($options['xmlNamespaces']) ? \Webmention_Comments\Masterminds\HTML5\Parser\Tokenizer::CONFORMANT_XML : \Webmention_Comments\Masterminds\HTML5\Parser\Tokenizer::CONFORMANT_HTML);
        $parser->parse();
        $this->errors = $events->getErrors();
    ...
    public function save($dom, $file, $options = array())
    {
        ...
        $options = \array_merge($this->defaultOptions, $options);
        $rules = new \Webmention_Comments\Masterminds\HTML5\Serializer\OutputRules($stream, $options);
        $trav = new \Webmention_Comments\Masterminds\HTML5\Serializer\Traverser($dom, $stream, $rules, $options);
        $trav->walk();
        if ($close) {
            \fclose($stream);
        }
