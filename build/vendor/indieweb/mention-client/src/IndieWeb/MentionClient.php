...
namespace Webmention_Comments\IndieWeb;
...
class MentionClient
{
    ...
    public $usemf2 = \true;
    // for testing, can set this to false to avoid using the Mf2 parser
    /**
    ...
    public function discoverPingbackEndpoint($target)
    {
        if ($this->c('supportsPingback', $target) === null) {
            ...
            if (\array_key_exists('X-Pingback', $headers)) {
                ...
                $this->c('supportsPingback', $target, \true);
            } else {
    ...
    {
        if (\class_exists('Webmention_Comments\\Mf2\\Parser') && $this->usemf2) {
            $parser = new \Webmention_Comments\Mf2\Parser($html, $target);
    ...
    protected function _findWebmentionEndpointInHTML($body, $targetURL = \false)
    {
        ...
        if ($endpoint !== \false && $targetURL && \function_exists('Webmention_Comments\\Mf2\\resolveUrl')) {
            // Resolve the URL if it's relative
            $endpoint = \Webmention_Comments\Mf2\resolveUrl($targetURL, $endpoint);
        }
        return $endpoint;
    }
    /**
    ...
    protected function _findWebmentionEndpointInHeader($link_header, $targetURL = \false)
    {
        ...
        if ($endpoint && $targetURL && \function_exists('Webmention_Comments\\Mf2\\resolveUrl')) {
            // Resolve the URL if it's relative
            $endpoint = \Webmention_Comments\Mf2\resolveUrl($targetURL, $endpoint);
        }
        return $endpoint;
    ...
    public function discoverWebmentionEndpoint($target)
    {
        if ($this->c('supportsWebmention', $target) === null) {
            ...
            if ($link_header && ($endpoint = $this->_findWebmentionEndpointInHeader($link_header, $target))) {
                ...
                $this->c('supportsWebmention', $target, \true);
            } else {
    ...
    public static function sendWebmentionToEndpoint($endpoint, $source, $target, $additional = array())
    {
        ...
        $payload = \http_build_query(\array_merge(array('source' => $source, 'target' => $target), $additional));
        return static::_post($endpoint, $payload, array('Content-type: application/x-www-form-urlencoded', 'Accept: application/json, */*;q=0.8'));
    }
    /**
    ...
    public function sendMentions($sourceURL, $sourceBody = \false)
    {
        if ($sourceBody) {
            ...
            $parsed = \Webmention_Comments\Mf2\parse($this->_sourceBody, $sourceURL);
    ...
    protected static function _head($url, $headers = array())
    {
        ...
        return array('code' => \curl_getinfo($ch, \CURLINFO_HTTP_CODE), 'headers' => self::_parse_headers(\trim($response)), 'url' => \curl_getinfo($ch, \CURLINFO_EFFECTIVE_URL));
    }
    /**
    ...
    protected static function _get($url, $headers = array())
    {
        ...
        return array('code' => \curl_getinfo($ch, \CURLINFO_HTTP_CODE), 'headers' => self::_parse_headers(\trim(\substr($response, 0, $header_size))), 'body' => \substr($response, $header_size), 'url' => \curl_getinfo($ch, \CURLINFO_EFFECTIVE_URL));
    }
    /**
     * @param $url
     * @param $body
    ...
    protected static function _post($url, $body, $headers = array())
    {
        ...
        $header_size = \curl_getinfo($ch, \CURLINFO_HEADER_SIZE);
        return array('code' => \curl_getinfo($ch, \CURLINFO_HTTP_CODE), 'headers' => self::_parse_headers(\trim(\substr($response, 0, $header_size))), 'body' => \substr($response, $header_size));
    }
    /**
