 ...
 */
class MentionClientTest extends \Webmention_Comments\IndieWeb\MentionClient
{
    public static $dataDir = null;
    private static $_redirects_remaining;
    public function __call($method, $args)
    {
        $method = new \ReflectionMethod('Webmention_Comments\\IndieWeb\\MentionClient', $method);
    ...
    protected static function _head($url, $headers = array())
    {
        ...
        return array('code' => $response['code'], 'headers' => $response['headers'], 'url' => $response['url']);
    }
    ...
    private static function _read_file($url)
    {
        ...
        if (($code == 302 || $code == 301) && \array_key_exists('Location', $parsedHeaders)) {
            $effectiveUrl = \Webmention_Comments\mf2\resolveUrl($url, $parsedHeaders['Location']);
            if (self::$_redirects_remaining > 0) {
                self::$_redirects_remaining--;
                return self::_read_file($effectiveUrl);
            } else {
                return ['code' => 0, 'headers' => $parsedHeaders, 'body' => $body, 'error' => 'too_many_redirects', 'error_description' => '', 'url' => $effectiveUrl];
            }
        } else {
            $effectiveUrl = $url;
        }
        return array('code' => $code, 'headers' => $parsedHeaders, 'body' => $body, 'url' => $effectiveUrl);
    }
}
